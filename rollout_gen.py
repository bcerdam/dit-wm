...
#         if model:
#             action, _ = model.predict(img_obs, deterministic=False)
#             action = int(action)
#         else:
...
#         print(f"Loading agent for rollout: {policy_path}")
#         model = PPO.load(policy_path, device='cuda')
#     else:
#         print("Using Random Policy for rollout.")
...
# Now accepts stack_frames argument
def run_episode(env, model, pixel_space, max_steps=1000, stack_frames=4):
    env = FireResetEnv(env)
    obs, _ = env.reset()
    
    # --- DYNAMIC FRAME STACKING ---
    frame_stack = deque(maxlen=stack_frames)
    
    # 1. Prepare initial frame
    init_img = cv2.resize(obs, (64, 64), interpolation=cv2.INTER_AREA)
    init_ch_first = np.transpose(init_img, (2, 0, 1)) # (3, 64, 64)
    
    # 2. Fill stack with duplicates of the first frame
    for _ in range(stack_frames):
        frame_stack.append(init_ch_first)
    # ------------------------------
    ...
    while not episode_over:
        ...
        if model:
            # 1. Update Stack
            current_ch_first = np.transpose(img_obs, (2, 0, 1))
            frame_stack.append(current_ch_first)
            
            # 2. Construct Input: (stack_frames * 3, 64, 64)
            agent_input = np.concatenate(list(frame_stack), axis=0)
            
            # 3. Predict
            action, _ = model.predict(agent_input, deterministic=False)
            action = int(action)
        else:
            action = env.action_space.sample()
        # ------------------------
...
# ... (batch_encode, process_pixels_only, save_to_h5 remain the same) ...
def batch_encode(frames, vae, device):
    batch_np = np.stack(frames)
    with torch.no_grad():
        t = torch.tensor(batch_np).float().permute(0, 3, 1, 2).to(device)
...
# Now accepts stack_frames argument
def env_rollout(env_name, n_steps, vae, dataset_path, policy_path=None, stack_frames=4):
    gym.register_envs(ale_py)
    ...
    model = None
    if policy_path and os.path.exists(policy_path):
        print(f"Loading agent for rollout: {policy_path} (Stack: {stack_frames})")
    ...
    while collected_steps < n_steps:
        # Pass the stack_frames param here
        frames, actions, rewards, term = run_episode(env, model, pixel_space, stack_frames=stack_frames)
